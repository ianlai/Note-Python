class Solution:
    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        if not candidates:
            return []
        ansArr = []
        self.dfs(candidates, target, ansArr, [], 0)
        return ansArr
        
    def dfs(self, candidates, target, ansArr, curArr, index):
        if target < 0:
            return 
        if target == 0:
        ...
        if target > 0:
            for i in range(index, len(candidates)):
                ...
                self.dfs(candidates, target-candidates[i], ansArr, curArr+[candidates[i]], i)
            return
